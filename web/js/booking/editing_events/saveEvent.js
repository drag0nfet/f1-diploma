...
export function saveEvent() {
    ...
    if (!dateTimeRegex.test(timeStart)) {
        showNotification(
        "error",
        "Некорректный формат даты и времени. Используйте формат дд.мм.гггг чч:мм"
        )
        return;
    }
    ...
    fetch("/save-event", {
        ...
        .then(data => {
            if (data.success) {
                ...
                showNotification(
                    "error",
                    `Ошибка сохранения ивента:${data.message || "Неизвестная ошибка"}`
                )
            }
        })
        .catch(error => {
            ...
            showNotification(
                "error",
                `Не удалось сохранить ивент: ${error.message}`
            )
        });
}